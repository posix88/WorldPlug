...
**Still to come** (agreed with the owner, not yet implemented): Chunk B restructures the Pack Check tab into a "Trips" tab with a create-trip-then-add-devices flow (list → `TripEditorView` for destination/dates/name → `TripDetailView` owning devices and verdicts); Chunk C strips the Saved tab to a plain favorites list, moves the favorite-widget picker into a new full-screen Settings screen behind a gear in the Countries toolbar, and moves the saved-country soft lock (free = 3) to the save action in Countries/CountryDetail.
